...
public class TeamPositifsBMain extends Brain {
    //---PARAMETERS---//
    ...
    private static final int TURNFULL= 4;
    private static final int TOPTASK= 5;
    private static final int STOPTASK= 6;
    ...
    //---CONSTRUCTORS---//
    public TeamPositifsBMain() { super(); }
    ...
    public void step() {
        ...
        for (IRadarResult o: detectRadar()){
            if (o.getObjectType()==IRadarResult.Types.OpponentMainBot || o.getObjectType()==IRadarResult.Types.OpponentSecondaryBot) {
                ...
                double rad = Math.atan2(enemyY,enemyX)*(180 / Math.PI);
                if(isSameDirection(getHeading(),rad)){
        ...
        if (state==TURNSOUTHTASK && !(isSameDirection(getHeading(),Parameters.WEST))) {
        ...
        if (state==TURNLEFTTASK && !(isSameDirection(getHeading(),oldAngle+Parameters.RIGHTTURNFULLANGLE))) {
            stepTurn(Parameters.Direction.RIGHT);
            return;
        }
        if (state==TURNLEFTTASK && isSameDirection(getHeading(),oldAngle+Parameters.RIGHTTURNFULLANGLE)) {
        ...
        if(state==TOPTASK && !(isSameDirection(getHeading(),Parameters.WEST))){
            stepTurn(Parameters.Direction.RIGHT);
            return;
        }
        if(state==TOPTASK && (isSameDirection(getHeading(),Parameters.WEST))){
            move();
            myX--;
            if(whoAmI==GAMMA && myX<=-300) {
                state=STOPTASK;
                return;
            }
            if(whoAmI==BETA && myX<=-450) {
                state=STOPTASK;
                return;
            }
            if(whoAmI==ALPHA && myX<=-600) {
                state=STOPTASK;
                return;
            }
        }
        if(state==TURNFULL){
//            System.out.println("passage : " + whoAmI);
            stepTurn(Parameters.Direction.SOUTH);
            myMove();
            if(whoAmI==ALPHA && myX<=80) {
                state = TOPTASK;
                return;
            }
            if(whoAmI==BETA && myX<=80) {
                state = TOPTASK;
                return;
            }
            if(whoAmI==GAMMA && myX<=80) {
                state = TOPTASK;
                return;
            }
            return;
