...
public class Bot
{
  ...
  public Bot(double radius, double range, double speed, double stepTurnAngle, double x, double y, double angle, double health, boolean rocket, Brain brain, int me)
  {
    this.radius = radius;
    frontRange = range;
    this.speed = speed;
  ...
  protected void move()
  {
    ...
    if ((newX >= radius) && (newX <= engine.getWorld().getWidth() - radius) && (newY >= radius) && (newY <= engine.getWorld().getHeight() - radius)) {
      ...
      for (Bot bot : engine.getBots()) {
        if (!bot.equals(this))
          if ((newX - bot.getX()) * (newX - bot.getX()) + (newY - bot.getY()) * (newY - bot.getY()) < (getRadius() + bot.getRadius()) * (getRadius() + bot.getRadius())) doIt = false;
      }
      if (doIt) {
        x = newX;
        y = newY;
